...
#
# Override: $env:MAP_INSTALL_TARGETS = "claude"  # or "grok" or "claude,grok"
...
$targetsCsv = if ($env:MAP_INSTALL_TARGETS) { $env:MAP_INSTALL_TARGETS } else { 'claude,grok' }
...
function Install-MapSkillLink {
    ...
    if (Test-Path $target) {
        ...
        if (Test-IsLinkLike $item) {
            # Junction / symlink: remove the link only
...
foreach ($raw in ($targetsCsv -split ',')) {
    ...
    switch ($name) {
        ...
        default  { throw "Unknown install target '$name' (use claude and/or grok)." }
    }
}
